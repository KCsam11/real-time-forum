...
export function setupMsgNotif() {
  ...
  function updateMessageNotificationDisplay(notifications) {
    if (msgNotifCount) {
      const unreadCount = notifications.filter((notif) => !notif.read && notif.type === 'message').length;
    ...
    notifications.forEach((notif) => {
      ...
      msgNotifItem.addEventListener('click', () => {
        ...
        // openConversation(notif.sender); // Ouvre la conversation avec l'expéditeur
      });
